...
EV="$S/evidence/run3-negative-charged-at-base-ee94c37"
mkdir -p "$EV"
GAME_ID="$(sed -n 's/^game_id=\([0-9]*\).*/\1/p' "$S/evidence/run1-charged-ee94c37/fixture.log")"
cd "$S/base/backend"
...
time python3 manage.py replay_round --game-id "$GAME_ID" --round 1 \
  ...
  --expected-manifest "$S/evidence/run1-charged-ee94c37/recorded/expected-manifest.json" \
  --evidence-dir "$EV" \
