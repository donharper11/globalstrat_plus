...
def wait_processed(page, timeout=None):
    # WALKTHROUGH 4: a refused processing attempt answers 400 at once and
    # then this loop waits out the whole window for a status that will
    # never change. The window is settable so a run that has to retry the
    # close does not spend fifteen minutes per attempt.
    timeout = timeout or int(os.environ.get('WAIT_PROCESSED_SECONDS', '900'))
    t0 = time.time()
    last = None
...
def main():
    with sync_playwright() as p:
        ...
        # WALKTHROUGH 4. Which teams pressed the button THEMSELVES has to be
        # read before the close, because "Close round now" locks every
        # outstanding submission -- that is what it is for, and afterwards
        # every team reads `locked` whether it submitted or was submitted.
        # The whole verdict turns on this number, so it is taken first.
        pre_dash = api(page, 'GET',
                       '/api/games/%s/instructor/dashboard/' % GID)['body']
        before_rows = [(t.get('team_name'), t.get('decision_status'))
                       for t in ((pre_dash or {}).get('teams') or [])]
        R.observe('team_status_before_close', before_rows)
        R.observe('locked_themselves',
                  [n for n, s in before_rows if s == 'locked'])
        R.observe('pending_at_close',
                  [n for n, s in before_rows if s != 'locked'])
