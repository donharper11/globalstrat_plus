...
def main():
    with sync_playwright() as p:
        ...
        ok = sign_in(page, '/login', STUDENT, LANG, password=members[0].get('student_id') or STUDENT, rec=R)
        ...
        if switch.count():
            ...
            # WALKTHROUGH 4. The switch now WRITES TO THE SERVER, so this
            # check changes the reader's stored enrolment language -- and the
            # team language rule (R43, decision 17) reads the FIRST
            # enrolment, which is this very student. Restoring localStorage is
            # not enough any more: the server is put back and read back.
            restored = switch_language_in_product(page, LANG, rec=R)
            R.step('the language this tour borrowed is given back on the server',
                   'pass' if restored == LANG else 'fail',
                   'server now %r, wanted %r' % (restored, LANG))
        else:
