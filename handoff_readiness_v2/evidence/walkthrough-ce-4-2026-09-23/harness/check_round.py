...
CHARGE_LINES = ['total_cogs', 'rd_expense', 'platform_amortization',
                ...
                'inventory_expense', 'other_operating_expense',
                'interest_expense', 'tax_expense',
                'other_non_operating_expense']

# WALKTHROUGH 4. The page's line list is READ FROM THE PAGE'S OWN SOURCE
# (frontend/.../pages/incomeStatementRows.js) instead of being copied into
# this driver, so the check cannot pass merely because the harness was edited
# to match a repair. Walkthrough 3 hard-coded the eleven lines the page
# printed then; the W-CE3-03 repair changed that list.
_ROWS_JS = (SCRATCH.parents[3] / 'frontend' / 'globalstrat-frontend' / 'src'
            / 'pages' / 'incomeStatementRows.js').read_text()
_LINES = re.findall(
    r"\{\s*key:\s*'([A-Za-z0-9_]+)'\s*,\s*field:\s*'([A-Za-z0-9_]+)'", _ROWS_JS)


def _sum_spec(name):
    m = re.search(name + r":\s*\[([^\]]*)\]", _ROWS_JS)
    return set(re.findall(r"'([A-Za-z0-9_]+)'", m.group(1))) if m else set()


_SKIP = ({'gross_profit', 'net_income'} | _sum_spec('revenue')
         | _sum_spec('subtotals') | _sum_spec('ratio'))
PAGE_LINE_FIELDS = [f for k, f in _LINES]
PAGE_OPEX = [f for k, f in _LINES if k not in _SKIP]
out['page_income_statement_lines'] = PAGE_LINE_FIELDS
out['page_income_statement_charges'] = PAGE_OPEX
print('the page prints %d lines; %d of them are charges: %s'
      % (len(_LINES), len(PAGE_OPEX), ', '.join(PAGE_OPEX)))
out['reconciliation'] = {}
for name, t in out['teams'].items():
    ...
    served_but_unshown = sum(g(k) for k in CHARGE_LINES
                             if k not in PAGE_OPEX and k != 'total_cogs')
