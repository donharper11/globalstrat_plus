...
def set_budget(page):
    """Set a budget allocation if the round has none.

    WALKTHROUGH 4. `student_play.py` drives the four teams that play in full;
    this driver also finishes the round for two teams that only submit, and
    for those the Summary refuses with *Set the budget allocation before
    locking.* -- correctly, because it is a required decision. The driver now
    does what that sentence tells a student to do, from the Finance page, so
    six of the eight teams submit a round rather than four.
    """
    stored = (api(page, 'GET', DEC)['body'] or {}).get('budget_allocation')
    if stored and any(float(stored.get(k) or 0) for k in
                      ('rd_budget', 'marketing_budget', 'strategy_budget')):
        R.step('a budget allocation is already in this round', 'observed',
               json.dumps(stored, ensure_ascii=False)[:200])
        return
    ...
    ctx = api(page, 'GET', '/api/games/%d/teams/%d/context/finance/'
              % (GID, TID))['body']
    budget = float((ctx or {}).get('operating_budget') or 0) or 5000000.0
    share = int(budget // 3)
    boxes = pane(page).locator('input.ant-input')
    if boxes.count() < 3:
        R.step('the budget allocation boxes are offered', 'fail',
               '%d inputs on the Finance page' % boxes.count())
        return
    for i, value in enumerate((share, share, budget - 2 * share)):
        set_number(page, boxes.nth(i), int(value))
        page.wait_for_timeout(400)
    page.wait_for_timeout(3500)
    after = (api(page, 'GET', DEC)['body'] or {}).get('budget_allocation') or {}
    R.observe('budget_allocation', after)
    R.step('a budget allocation was set from the Finance page',
           'pass' if any(float(after.get(k) or 0) for k in
                         ('rd_budget', 'marketing_budget', 'strategy_budget'))
           else 'fail', json.dumps(after, ensure_ascii=False)[:200])
    R.screen(page, 'x-t%d-r%d-90-budget' % (TEAM_IX, ROUND))
