...
def market_strategy(page):
    ...
    if d0.get('plant_decisions'):
        ...
        # The decision row's capacity_units is an ADDITION to the market's
        # authored plant_capacity_units, which is what the engine builds with
        # (engine/strategy_effects.py:288). A row of 0 is therefore correct and
        # the walkthrough-2 version of this check was mis-specified; what is
        # asserted is that the button's promised capacity is the scenario's.
        cap_promised = (R.record['observed'].get('build_plant_label') or '')
        R.step('market strategy: the Build Plant control names the capacity the scenario authors',
               'pass' if any(ch.isdigit() for ch in cap_promised) and pl else 'fail',
               'label=%r stored=%s' % (cap_promised, json.dumps(pl)[:200]))
    else:
...
def corporate(page):
    ...
    # W-CE2-02: since the repair an Acquire button is rendered only for a
    # target that is actually available (round reached, presence held, not
    # already taken); an unavailable target shows its requirement tags and an
    # unaffordable one is disabled with the figures beside it. So the absence
    # of a button is read off the card, not treated as a missing control.
    ma_ctx = api(page, 'GET', '/api/games/%d/teams/%d/context/strategy/' % (GID, TID))['body']
    targets = (ma_ctx or {}).get('acquisition_targets') or []
    R.observe('acquisition_targets', [{k: tg.get(k) for k in
              ('target_name', 'market_name', 'base_acquisition_cost',
               'min_round_available', 'available', 'locked_reasons')}
              for tg in targets])
    R.observe('ma_pane_text', pane(page).inner_text()[:1500])
    acq = pane(page).locator('button', has_text=T('corporate_strategy.acquire'))
    available = [tg for tg in targets if tg.get('available')]
    if acq.count():
        ...
        if ok:
            ...
            R.step('corporate: every offered acquisition is disabled and says why', 'observed',
                   'buttons=%d; unaffordable notice on the card=%r'
                   % (acq.count(),
                      T('corporate_strategy.not_affordable') in (R.record['observed'].get('ma_pane_text') or '')))
    elif not available:
        R.step('corporate: no acquisition target is available yet, and each says why',
               'pass' if targets and all(tg.get('locked_reasons') for tg in targets) else 'fail',
               '%d targets, none available; reasons=%s'
               % (len(targets), json.dumps([tg.get('locked_reasons') for tg in targets], ensure_ascii=False)[:400]))
    else:
        R.step('corporate: an available acquisition target carries an Acquire control', 'fail',
               json.dumps([tg.get('target_name') for tg in available])[:200])
    # ESG
...
def main():
    with sync_playwright() as p:
        ...
        ok = sign_in(page, '/login', STUDENT, LANG, password=SID, rec=R)
