...
absent `.aide-checks-rev`) is a tooling observation reported identically by four
of the five handoffs; it is recorded in
`completion/REGISTER_BACKLOG_2026-09-12.md` rather than as a finding, because it
describes the checks runner and not this product.
...
## V2-086 through V2-094 — the paid-research merge, and a cash path found beside it (2026-09-12)

**Chronology.** These nine follow the V2-075–V2-085 block and were raised after
...
defect that report's own finding 7 flagged in passing. As with the previous
...
| R28 | V2-085's rank-churn observation; GSP-CRV2-11 Stage 2 | **Author more starting profiles, so no two firms in a heat begin from the same position** — enough for a full 8-firm heat in all three scenarios. **R22 still binds and is not relaxed:** every team opens on the same index and rank; profiles differ in position, not in opening score. Balance must be **measured, not asserted**. A new authoring task, opened 2026-09-12 — not registered as a finding here. |
