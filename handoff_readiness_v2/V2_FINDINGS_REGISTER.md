...
| R34 | V2-119 **question 2 — now ruled**, superseding the row above | **Ruled and implemented.** A guard firing is recorded as a `DecisionAuditEvent` (`action=inactivity_rank_demotion`, `endpoint=engine:update_leaderboard`, actor `system`) carrying the firm's index, the lowest active index and team name, the ranks, and `outscored_a_firm_ranked_above`. **Deliberately not a field on the `performance` or `leaderboard` rows:** both are hashed output sections, so a field there would move the envelope v6→v7 exactly as paid research did, invalidating replay evidence for a control that has never fired. Implemented at `6730168`, merged at `1e990cf`. **Audited rather than accepted:** the change is confined to `leaderboard.py` (+130, no deletions) with **no model change and no migration**, `MANIFEST_SCHEMA_VERSION` is still **6** (`services/manifest_version.py`, verified at the commit — an earlier check of mine grepped the wrong file and proved nothing), and R32's 14-test module is untouched. The builder also **corrected an assumption of mine**: the price-band precedent does not transfer, because those receipts are written before `prepare_manifest` snapshots the input while these are written after it — so it verified the recorded input holds no demotion row, and proved the receipt is *re-written* by the replayed engine rather than merely ignored by the hash. Replay over a round where the guard actually fires: input verified, competitive hash expected == actual, no section diffs, exit 0. Merged regression 212 tests, OK. |
| R35 | V2-119 — user-facing remainder | **Ruled and implemented.** `RoundResultsView` returns `inactivity_notices` beside `price_adjustments`, rendered **from the stored R34 payload** so the sentence a team reads and the row an instructor produces are the same fact. Bilingual, naming no column, code or id, and stating that **the index itself was not reduced — only the placing**, which is R32's whole point. **Two message variants keyed on R34's `outscored_a_firm_ranked_above`,** so a firm that would have finished last anyway is not told it lost a place it never held — screen and stored record cannot contradict each other. Implemented at `98af8eb`, merged at `f8c4b47`. Given F3's history ("the API is right and nothing renders it") the firing was constructed — it has never occurred in 448 stored rounds — and driven in real Chromium in both languages, quoted from the live DOM, with screenshots. Cross-team leakage re-asserted with both teams demoted so there was genuinely something to leak: rival student **403** (status asserted, not a bare absence), owner 200. No model change, no migration, envelope still **6**; R32's and R34's test modules absent from the diff. Merged regression 165 tests OK; participant-string gate PASS, 2,189 units. **V2-119 is closable by the auditor on these three rulings; no closure is claimed here.** |
| R38 | V2-110 **remaining question — now ruled**, superseding the R32 row above | **Ruled 2026-09-21; no engine change.** The adopted V2-022 supplementary disposition stands — a compliance-frozen team below the material revenue floor is commercially inactive and the controls stack — **on the condition that a freeze blocks sales only in the market it was imposed in** and never in another market where the team is active and compliant. Owner's words: "If it is only affecting the market it's frozen out of, then ok. It must not affect all other markets in which the team is active and compliant." The condition is pinned by `test_cc18_compliance.py::test_a_freeze_in_one_market_leaves_the_other_market_to_the_cent` (two markets, one frozen, compared to an unfrozen control: compliant market identical, team not inactive), **falsified** by widening `revenue.py`'s freeze test to the whole team (test fails; engine restored, absent from the diff). Module 16 tests OK. Firm-level effects of a freeze (P&L penalty, 0.12 stakeholder and 0.18 execution-resilience deductions per freeze, an unread `reputation_impact`) were described to the owner and are **not ruled on**. Record: `OWNER_RULINGS_2026-09-21.md`. |

## V2-121 and V2-122 — registered 2026-09-17

Both found by the R35 builder while proving its own work, and **both verified by
me before registration** rather than taken from the report — which mattered,
because my counter-hypothesis was the one that turned out to be wrong.
...
### Release-identity branch merged 2026-09-21 (`f0e26a7`), and its caveats worked rather than listed

| Item | State |
|---|---|
| V2-124 (code half) | **Repaired, pending closure; the finding stays OPEN until the deployment half is done.** With the clean-build flag on, resolution refuses unless the advertised revision is a full commit hash and, from a checkout, equals HEAD with a clean tracked tree; a checkout git cannot read is refused rather than read as an immutable build; the source digest is taken at process start and resolution refuses if the disk has since changed. One implementation behind the guard and `check_release_identity`. First tests for `check_release_identity` and `resolve_stored_revision`. **Deploying this while production's `GIT_REVISION` is still wrong makes production refuse to resolve** — by design; the operator procedure is `completion/RELEASE_IDENTITY_GUARD_2026-09-21.md` §6 and is one maintenance action. Commit-map translation in recovery deliberately not built (the V2-048 rewrite changed file contents, so a translated commit is not provably the same bytes). |
| V2-125 | **Repaired, pending closure.** `requirements.txt` joins the source digest; `runtime_config.py` records an allow-list of non-secret effective settings and their digest in `ResolutionManifest.environment`, outside the hashed envelopes — `MANIFEST_SCHEMA_VERSION` still 6, pinned by a test that resolves a round; a test rotates only the secrets and the digest does not move. **Residual, P2:** `environment` is outside the audit chain, so the record is not tamper-evident. |
| V2-123 residual (1) | **Repaired, pending closure** at `3d3a0a2`. Worse than the re-audit described: the default was `skip_rag=False`, so a forgotten argument was enough to blend a model score. Default is now `True`; `False` raises an R31 refusal before any write. |
| Builder caveat: "no before-and-after output hash was run" | **Closed by execution.** Two rounds recorded on a disposable stack by the engine at `999332c` (pre-merge), round 2 replayed by the engine at `62e2326` with `--allow-source-mismatch`: competitive `106e2cb3…` and narrative `02a4b0ee…` both expected == actual. The merge is byte-identical on the production path. |
| Builder caveat: the start-of-process drift check refuses if anything writes `.json`/`.yaml` under `backend/` | **Checked, and the one real gap closed** at `62e2326`. Manifest bodies are written under the backup root, which the digest excludes, and no other runtime writer of those suffixes exists under `backend/` (every such file there is tracked source). But the exclusion was by directory *name*: a differently named `COMPETITION_BACKUP_DIR` inside the tree would have made every resolution refuse the next. The configured directory is now excluded as well; default digest unchanged; test with a positive control. |
| V2-120's cell | **Correction:** its sentence "no new manifest can join this set" was not true when written — production could still stamp a wrong revision. It becomes true once V2-124 is deployed. |
| Not changed, stated: | Under the flag, an immutable build must advertise a full hash, so a tag-named build is refused (stricter than before; one line to relax). One pre-existing test, `test_clean_build_gate_accepts_a_named_revision`, was rewritten because it asserted the defect as the expected behaviour. |

### Last two builder branches merged 2026-09-21; full suite green on the merged tree

**Full backend suite at `f46d173`: `Ran 1213 tests`, OK, rc=0, 99 s** (`scripts/test-postgres core --parallel 8`); 1209 OK one merge earlier at `a571186`. Frontend Jest 15 suites / 94 tests; participant-string gate PASS at 4,679 units; string inventory `--check` clean. **Not the freeze-candidate run** — the checklist gate stays unticked for the reason recorded on 2026-09-17.

| Finding | State |
|---|---|
| V2-112 | **Repaired, pending closure** at `ffd34a9`. One `game_creation.create_game` behind both paths. The register's premise was wrong: the web path had never worked (500 for every JWT instructor — a `JWTUser` assigned to an `auth.User` key) and a game shaped its way could not resolve round 1. Command-built starting state byte-identical before and after across 3 scenarios × 8 firms. **Open against the owner:** the authored `beta` platform blocks are unused on every path. |
| V2-114 | **Partly repaired; stays OPEN against the owner.** Proposed references — Clean Energy 2200/3700/8900/13000, Media 71/120/270/390, Consumer Electronics byte-unchanged — cut clamped starter products from 16 to 6 and 16 to 1. Clean Energy's mainstream tier spans 9.4× and the score is live across only 3×, so no single reference holds more than six of its ten products live. `OWNER_DECISIONS_PENDING` D11 says only the owner sets these numbers: **these are a proposal in the tree, not a ruling.** Rounds-1+ balance has shifted in both scenarios and has not been measured (R28). |
| Console and analyst leftovers | **Repaired, pending closure** (`05967b2`, `b244e73`, `4856d89`): 46 hard-coded English console announcements moved to the catalogues with a source-scan guard; `under_minimum` shown as a standing warning; analyst 429/503 bilingual, the 503 no longer leaks exception text. |
| New, assigned same day | Hard-coded Consumer-Electronics `coherence.PRICE_RANGES` (positioning-price coherence is 0.0 for every Clean Energy product) and a manifest test that depends on `Team` ids → `crv2-11-coherence-price-ranges`. Unverified sweep claims — no ownership check on `PUT roster/`, `str(e)` leaks, `GameDeleteView` outside the lifecycle boundary → `crv2-08-operator-route-ownership`. Orphan purchase audit event on a failed analyst query, English-only analyst and operator refusals, CSV roster upload discarding per-row errors, 比赛/游戏 inconsistency → `crv2-12-analyst-audit-and-roster-upload`. |

### Coherence price ranges merged 2026-09-21 (`48f032f`)

| Item | State |
|---|---|
| Hard-coded `coherence.PRICE_RANGES` (the V2-114 builder's N3, proposed P1) | **Repaired as a proposal, pending the owner's confirmation of the derivation rule.** Each bound keeps the ratio to its tier's reference price that it has in Consumer Electronics (budget 2/5–6/5, mainstream 25/42–55/42, premium 5/7–9/7, ultra-premium 4/5–3/2), applied to the scenario's own references; no number introduced; exact `Fraction` arithmetic so Consumer Electronics derives the same integers, which matters because the range string is inside a hashed section. Starter products scoring above 0.0: Clean Energy 0 → 10 of 16, Media 0 → 15 of 16; the seven left are exactly V2-114's residual clamps. A missing reference or unknown positioning now refuses. **Byte-identity checked independently by the integrator:** a Consumer Electronics round recorded at `a95cc62` replayed under `48f032f` with `--allow-source-mismatch` — competitive `353f91ca…` and narrative `49aaee85…` both expected == actual. The builder's one open worry (resolved Clean Energy or Media rounds in production whose hashes would stop replaying) is answered by R41: nothing has been live. |
| Manifest test dependent on `Team` ids (N4, proposed P2) | **Repaired; a fixture defect, not a product defect.** The fixture sourced half of each team's supply from a Xinjiang-adjacent supplier, so every team took the UFLPA enforcement draw, which is seeded on `Team.id` by design (V2-011). At ids 135–137 all three draws fall under 0.15 and every team is frozen out of NA, leaving no demand rows. The fixture now sources from unflagged suppliers, pinned by a test that forces every draw to fire and one that rebuilds the fixture at those ids. |

## V2-131 through V2-136 — operator-route ownership and the second-wave repairs (registered 2026-09-21)

An unverified read-only sweep made three claims about operator routes. Each was driven through the real routes with two instructor accounts before anything was repaired; all three were true, and the builder's standing question "is there an alternate entry point?" found two holes the sweep had missed. **Full backend suite on the merged tree at `60c13fe`: `Ran 1335 tests`, OK, 100 s. Jest 18 suites / 141 tests. Participant-string gate PASS at 4,829 units.** Not the freeze-candidate run.

| ID | Area | Sev | Finding | Status |
|---|---|---|---|---|
| V2-131 | Authorisation / account routes | **P0** | **Any instructor could make themselves an admin.** The generic `users/` viewset let an instructor `PATCH` their own `role` to `admin`, create a new admin with a password of their choosing, and set another instructor's password. | **Repaired, pending closure** at `6b7972f`. For non-admins `users/` is scoped to their own students, role is fixed to Student, and an attempt to reach a staff role is recorded. **Not established: whether it was ever used.** That needs the production access log and a check for unprovisioned `role='admin'` accounts — an operator action, not yet done. R41 (nothing has been live) lowers the likelihood and does not replace the check. |
| V2-132 | Authorisation / team state | **P0** | **Any instructor could change any team's cash, unaudited**, through `PATCH /api/teams/<id>/ {cash_on_hand}`. | **Repaired, pending closure** at `6b7972f`: `teams/` is read-only; nothing in the product wrote through it and every sanctioned team write has a guarded route. Mutating routes 219 → 215. |
| V2-133 | Authorisation / cohort routes | **P0** competition, P1 pilot | **Cohort routes that name no game id had no ownership check.** A non-owning instructor read another's roster, renamed and removed students, uploaded a CSV, seated their own student on the other cohort's team, deleted the course, and set themselves its instructor of record. | **Repaired, pending closure** at `90e8540`. One helper (`cohort_scope.py`) applies the adopted rule to `roster/`, `team-management/`, `courses/`, `sections/`: admin always, an unowned course stays the shared pilot cohort, otherwise only the instructor of record. 403, bilingual, coded, and a refused write leaves an `AuthorizationRefusalEvent`. One foreign item in an assignment refuses the whole request. **Open against the owner:** the console creates courses with no instructor of record, so they stay shared until an admin assigns one. |
| V2-134 | Lifecycle boundary | **P1** | **Game delete sat outside the lifecycle boundary**: no lock, no reason, no audit, checked in as not lifecycle-mutating, deleted competition heats, and 500'd on any game with an audit row. The sweep's sub-claim of no ownership check was **false** — the game-scope guard already refused another instructor's game. | **Repaired, pending closure** at `58e6bd9`. Under `@lifecycle_view` and `operator_action('delete_game')`; a heat is refused (409 `competition_game_not_deletable`), a game with any record is refused (409 `game_has_record`), both pointing to archive; a reason is required. Lifecycle-mutating routes 37 → 38, guarded 21 → 22; the inventory detector now recognises `_delete_game_cascade`. **Open against the owner, low stakes:** a committed deletion is a `core.lifecycle` log line, not an audit-table row, because `OperatorAuditEvent` holds a protected key to the game; a durable row needs a new table with no game key. Only a game with no record at all can be deleted, which is what keeps this low. |
| V2-135 | Operator console | **P1** | **Reset and archive never worked from the console**: both buttons posted an empty body to routes that require a written reason, so every click returned 400. | **Repaired, pending closure** at `fa23936`: reset, archive and delete collect the reason through a modal (`ReasonedAction.js`). Not observed in a browser. |
| V2-136 | Exception text / language | **P1** | Roster routes and `users/bulk-upload/` returned `str(e)` to the client; operator routes refused in English only; a failed analyst query was believed to leave an orphan purchase audit event; the CSV roster upload discarded per-row refusals; zh-CN used three words for "game" and three for "team". | **Repaired, pending closure** (`43b9181`, `1942add`, `54b8f95`, `2cf0281`, `90e8540`). Exception text goes to the log with a reference id. `operator_messages.py`: 101 sentences, 72 stable codes; the frontend code list must equal the backend's or a test fails. The orphan-audit report was **wrong in its premise** — the whole request is one transaction, so no charge ever committed — and the audit event is now written only once the answer exists. One zh-CN term per concept, guarded by `test_zh_terminology`, **which on merge caught two retired terms in the other branch's new sentences.** **Remainder assigned same day** to `crv2-12-remaining-english-refusals`: 62 English literals outside the console's live-round routes, 48 of them student-facing. |
...
### R48 delegated calls merged 2026-09-22

**Full backend suite on the merged tree: `Ran 1519 tests`, OK. Jest 30 suites / 325 tests. Participant-string gate PASS.** Not the freeze-candidate run.

| Decision (`INTEGRATOR_DECISIONS_UNDER_R48.md`) | State |
|---|---|
| 7 — Chinese word limit | **Repaired, pending closure** (`a3ddab5`). CJK letters count at 1.5 per word on server and screen through one shared rule each side, sharing a 16-row case table. At head a 600-character Chinese memo against a 300-word limit was accepted at `word_count: 1`. |
| 8 — dead round-status routes | **Repaired** (`d86a699`). All three routes removed (the third, `rounds/current/my-status/`, shared the same failing view and had no caller), with the views, ten catalogue sentences only they spoke and one console code. Inventories regenerated: mutating 217 → 216, read-inventory routes 797 → 794. |
...
|---|---|---|---|---|
| W-CE-02 | **P0** | Finance, student | Typing `5000000` into Loan Amount stores $5, $0 or $50 depending on typing cadence; only paste works. | **Repaired, pending closure** (`e001c47`, merged). The Finance tabs were components declared inside the render, so every keystroke created new component types and React remounted the tab, destroying the focused input mid-word. Tabs are render functions now; no other page has the pattern. Driven key by key with the autosave live: 3 fail → 3 pass. |
| W-CE-14 | **P0** | Round Results, student | "Shareholder return 999,966.5 %" (API `shareholder_return_cumulative` 9999.665). | **Repaired, pending closure** (`6c53928`). **The number was wrong, not mis-scaled:** dollars of dividends were added to a per-share price, over a base of starting cash in millions instead of the round-0 share price — (33.25 + 500,000 − 50)/50 = 9,999.665 exactly. Per-share throughout now, base = the round-0 share price. **A hashed value moves** (`financials.shareholder_return_cumulative` and the derived `dividend_consistency` feature) for dividend-paying rounds; no section changes shape, envelope stays 7. |
| W-CE-01 | P1 | Game Control, instructor | Every action (activate, deadline, extend, process) throws the console back to Courses & Sections. | **Repaired, pending closure** (`457bfd6`, merged). A bare spinner during every reload unmounted the uncontrolled tabs, which reopened on their default. Active tab is now state; the spinner shows on first load only. |
| W-CE-03 | P1 | R&D, student | "Invest next level" offered five times; every click refused by the server. | **Repaired, pending closure** (`5c7d8c5`). The server now says feature-level investment is retired (bilingual) and the page no longer offers it; the guidance no longer says "upgrade a feature". W-CE-19 (the platform unaffordable in round 1) is calibration, deferred under R48. |
| W-CE-04 | P1 | Marketing, student | One bad row fails the whole page's save; the notice names no product and shows twice. | **Repaired within the contract** (`b0d3c35`): the save stays whole-page (a partial save would delete unreached rows), the refused row is named "{product} in {market}: {sentence}", shown once through the shared notice. |
| W-CE-05 | P1 | Roster upload, instructor | CSV upload announces nothing. | **Repaired, pending closure** (`93ede85`). The outcome helper was wired; a clean upload's only announcement was a three-second toast. The outcome now stays on the roster panel until dismissed. |
| W-CE-09 | P1 | Team Activity, student | Calls an instructor-only route; 403 on every visit. | **Repaired by removal** (`4926081`): page, route and sidebar entry gone (V2-106 precedent); a source scan keeps student pages off instructor-only routes. **Integrator decision pending:** whether a team may read its own change log at all. |
| W-CE-10 | P1 | Notifications, student | The bell is decorative; no notifications screen exists. | **Repaired by removal** (`9bde205`): nothing fed it. |
| W-CE-12 | P1 | Grading, instructor | Override has no console control. | **Repaired, pending closure** (`417bf93`): per-team override control with category, score and reason, clear, and recalculation; overridden categories tagged. |
| W-CE-15/16/17 | P1 | Many, zh-CN | English on Chinese screens: analyst refusal, R&D guidance, summary checklist, news and ticker narratives, login page; console tab "Students & Logins", Game Control statistics, Monitoring banner, Supply Chain panel, audit table. | **Repaired, pending closure** (branch `walk-ce-language`, 13 commits, merged). **W-CE-15's root cause was not a catalogue gap:** the login page's language choice was never recorded on the enrolment, so R43 read the default `en`; sign-in now records it. W-CE-16: view f-strings moved to the catalogue, research rating words code-driven, and the Phase-2 template fallbacks (briefing, supply-chain event, compliance, memo evaluation) now exist in Chinese and follow the team's language. W-CE-17: the whole console list. A sweep of the visited screens found 33 real English items beyond the named ids; 22 fixed; **11 remain, listed in `completion/WALK_CE_LANGUAGE_2026-09-22.md` §(d)** — chiefly scenario-authored English with no Chinese field (market outlook headlines, supply-chain instrument names, memo criteria), which needs authored Chinese content, not code. ~144 frontend keys and ~75 backend sentences await a native reviewer. **Open, rules:** R43 reads the first enrolment's language, so a Chinese-reading second team member can still be refused in English. |
| W-CE-18/18b | P1 | Summary / Finance, student | Strategy budget bar and charged expense disagree; the round-3 summary shows three different spend figures. | **Repaired, pending closure** (`e0cd389`). Committed spend counted declared budget lines and none of what the engine charges for the decisions themselves (acquisitions, entries, sales reps, payroll, plants), and Summary and Finance each summed "spent" privately. `rd_costs.budget_assessment` now reads the engine's own `decision_outlays`; declared lines are floors; every surface reads the one assessment. The 18b "counted again" was a second acquisition queued in round 3, not double counting. |
| W-CE-21 | P1 | Setup / student | Home market set by the instructor (Africa) shows "Not Entered"; the team starts in North America at VERY_HIGH cultural distance. | **Repaired, pending closure** (`0b3c1da`, merged). Cause: Team Configuration wrote only `Team.home_market`, while round 0 is built from the presence row at creation, so the console accepted a setting the game ignored. `rehome_team` now moves the rows creation keys on the home market and re-runs round-0 bootstrap in one transaction; a re-homed team is byte-identical to one created with that market; untouched games unchanged; R22 parity holds. Full suite on the merged tree: 1525 OK. Report: `completion/WALK_CE_HOME_MARKET_2026-09-22.md`. |
| W-CE-23 | P1 | Finance, student | $16.6M over-spend allowed in round 2; the team is then locked out in round 3 by negative cash it cannot undo. | **First half repaired** with W-CE-18 (the over-spend was uncounted spend). **Second half is a rules question, not decided:** with negative cash the affordability check compares spend to cash on hand and ignores decided financing, so it can never be satisfied — while the sentence beside it says "raise financing". The team is not stuck in the engine (its draft is deadline-locked and resolved, financing included) but can never press Lock. Should available cash count decided financing? Plain-language statement in `completion/WALK_CE_STUDENT_NUMBERS_2026-09-22.md` §8. |
| W-CE-24 | P1 | Lifecycle, instructor | "Advance Round" modal has no reason box and is refused `reason_required` whenever teams are pending. | **Repaired, pending closure** (`49d0447`): the reasoned modal when teams are pending, a plain confirm when all are locked; the server's bilingual refusal shown. |
| W-CE-25 | P1 | Summary, student | Lock refused for debt-to-equity 2.35 > 2.0 although the summary showed no blocker. | **Repaired, pending closure** (`ccb7ec1`): one validator, `lock_blockers_for`, feeds both the lock and the Summary's list, so the Summary shows exactly what the lock enforces (seven rules were lock-only before; the V2-024 funding check is now also refused at lock). EN and zh-CN. |
| W-CE-26 | P1 | Game Control, instructor | "Reset to Setup" accepted on a competition heat with four processed rounds; left at round 0 with rounds 1–4 still processed. | **Repaired, pending closure** (`263afb8`). Reproduced through the API. Reset is refused for a competition heat and for any game with a processed round (409, bilingual, audited, pointing at archive); an unplayed game still resets. No consistent reset existed to wire, so refusing was the least change. |
| W-CE-06/07/08/11/13/20/22 | P2 | Various — **all repaired, pending closure** (W-CE-07 `125bc9a`; the rest on `walk-ce-language`: the inject route stored the raw `{market}` template; status tokens labelled; audit table collapsed below the decision blocks; the strategy context never sent plant figures so every market showed $0 — the authored cost is now sent, "cost not set" when unauthored; supply-chain sections marked optional at lock; an in-game language switch using the existing preferences route) | `{market}` placeholder in the ticker; raw JSON in the Operator Log; raw `no_submission` tag; no in-game language switch; four supply-chain sections shown as lock requirements; drill-down leads with a giant audit table; "Build Plant — $0", partnership label vs stored per-round charge, `VERY_HIGH` enum on screen. | Open — assigned |
...
|---|---|---|---|
| W-CE2-01 | **P0** | A plant built in the same market as a completed acquisition writes two `team_plant` rows; post-round processing 500s and the game cannot advance. | **Repaired, pending closure** (`4693777`, merged). Reproduced to the exact `SnapshotError` on `team_plant`'s natural key. Both engine steps now write through `plants.record_plant`, which merges instead of colliding, so **the engine fix alone un-sticks a stuck round** — the operator re-runs post-round processing. The collision is refused at both saves and at lock, bilingually, naming the market. **The reproduction found a second, unrecorded game-stopper:** two build rows for one market break the input snapshot before Phase 1, which no engine repair can reach; refused at the save now, and cleared for an existing game by the tested `manage.py reconcile_plant_rows`. **Open for the owner:** which reconciliation a game already carrying a collision should get — combined capacity operational (implemented) or held under construction. |
| W-CE2-02 | P1 | A queued acquisition, plant or partnership can never be withdrawn; unaffordable ones are still offered and then refused at lock. | **Repaired, pending closure** (`8d9fda0`): each queued commitment is shown where it was made, with a withdraw control; the strategy context publishes affordability from the lock's own calculator, so an unaffordable offer is disabled with its figures. |
| W-CE2-03 | P1 | The deadline close executes the spend the lock refused: one team ended round 3 at −$12.4M. | **Repaired, pending closure** (`273daec`), narrowly and deliberately. No ruling exists on auto-lock at the deadline, and the whole lock validator cannot be applied there (most blockers are "section empty"). The builder implemented the one rule the refusal names, in the place the engine already withholds an acquisition for financial condition: an unaffordable submission's acquisitions are not fulfilled, nothing is charged, the team is told. **This changes a hashed value for that class of round.** **Open for the owner:** the uncovered case, where the overspend is a plant or marketing outlay rather than an acquisition. |
| W-CE2-10 | P1 | The first round-control action after Activate is always refused until the page is refreshed. | **Repaired, pending closure** (`5182912`): the round control card re-reads on activation. No guard weakened. |
| W-CE2-05 | P1 | A team's language is its **first** enrolment's, so a Chinese player is answered in English if a team-mate enrolled first. | **Repaired, pending closure** (`e3e5fc5`, merged). `language_for_team` had exactly one caller. A sentence addressed to one student — the analyst route's nine — now follows that student's own enrolment; anything written once for the team (briefing, compliance and supply-chain notices, memo evaluation, persona threads) keeps the team rule, and a test pins the split. **Open for the owner:** what language a team-wide document should use when members differ. |
| W-CE2-08 | P1 | AI Coach alerts are always English, because an instructor has no enrolment to read a language from. | **Repaired, pending closure** (`d4a3e21`). Two causes: console-created games record `created_by` as the **first superuser**, so the code read a superuser's language; and a console-created instructor has no enrolment, so the in-game language switch silently stored nothing. Migration `0090` adds a preference store (no engine reads it, no manifest section); alerts follow the owning instructor's stored preference at generation time. |
| W-CE2-04/06/07/09 | P2 | Raw `SnapshotError` in the Operator Log; English market names on Chinese screens; English Strategic Scorecard sentences; two round totals and an unformatted negative. | **All repaired, pending closure** (`370568f`, `61c6546`, `d2a5fa9`, `982ffad`). The scenario **does** author Chinese market names; four reads skipped the lookup. The scorecard sentence is a hashed field, so the stored English stays byte-identical and the reader re-derives the same key from the same stored numbers and renders a copy — no replay evidence invalidated. **Eight copies of a money formatter tested only `n >= 1e6`, so every negative fell through unformatted**; all eight fixed with a scan guard. |
| W-CE-18b | P1→P2 | Figures now agree; two round totals remain and one is unformatted. | **Repaired with W-CE2-09:** the panel states the one authoritative total, and the banner stops calling its three-line subset "total spending". No figure or threshold changed. |
| W-CE-23 | P1 | The refusal moved to the right round, but a team cannot clear it and the deadline spends anyway. | **Repaired with W-CE2-02/03** (withdraw control; the deadline no longer fulfils a refused acquisition). The negative-cash lock question stays open for the owner. |
| W-CE-24 | P1 | The reason box was added, but the route now refuses `round_not_ready`, so the button still cannot advance. | **Repaired, pending closure** (`7efd126`): with a reason on an open round the action closes the round first, which is what the processing route already does with force, satisfying the engine precondition rather than bypassing it; every guard that must not move now has a test. The modal's false "previous round's decisions will carry forward" is corrected in both languages. |
...
### Both walkthrough-2 repair branches merged — certifying run 2026-09-23

**Full backend suite on the merged tree at `29da1c8`: `Ran 1657 tests`, OK, 118 s. Jest 52 suites / 495 tests. Participant-string gate PASS at 5,755 units; inventory clean; build ratchet 54.** Not the freeze-candidate run. Every W-CE2 defect is repaired pending closure. **Added on merge:** a Jest timeout for the new activation test, which passed alone but exceeded the 5 s default under the full suite's parallel load — a flaky test would have bitten the freeze run. Next: the third Consumer Electronics walkthrough on this tree, then Media and Clean Energy.
...
### Walkthrough-3 repairs merged — certifying run 2026-09-23

**Full backend suite on the merged tree at `2576a65`: `Ran 1785 tests`, OK, 126 s. Jest 54 suites / 516 tests. Participant-string gate PASS at 5,916 units; inventory clean; build ratchet 54.** Not the freeze-candidate run.

| ID | State |
|---|---|
| W-CE3-02 + W-CE-23 | **Repaired, pending closure** (`a97f055`) under decision 13. `budget_assessment` never read the financing row. Available funds are cash plus the financing decided this round, through one new calculator the engine itself calls, so the lock cannot promise money the round declines; debt refused in distress is not counted and the team is told to raise equity. The refusal names the funds, the financing counted, the shortfall and the largest reducible commitment. **Whole-game proof:** before, a team at −$61.3M was force-advanced through rounds 3–6 and $61.6M of debt moved the figure not a cent; after, it was refused at round 3, raised the same amount as equity, locked, and recovered to +$18.7M — `total_forced_by_deadline=0`, `never_locked=[]`. |
| W-CE2-03 residue | **Repaired** (`57e9e4f`) under decision 14: `close_round` evaluates the lock's own assessment and withdraws discretionary commitments in a stated order until the draft fits, audited and with a bilingual notice. The order is the builder's, in the direction of least change, stated in one place. |
| W-CE3-01 | **Repaired** (`e91ead6`) under decision 15: the tax-structure setup charge goes through `decision_outlays` and `calculate_operating_expenses` with the assertion widened over it, exactly as R36 did. `annual_maintenance_cost` **checked and is not the same defect** — it sits inside `operating_income`, and moving it would change a published tax deduction (calibration, deferred). A pre-existing `UnboundLocalError` on `platform_switch_write_off` was fixed on the way. |
| W-CE3-03 / W-CE3-04 | **Page repaired, API complete, one part deliberately STOPPED.** Served fields close both gaps ($2,862,694.58 → $0.00 on the page, $615,289.00 → $0.00 served). **Naming depreciation, tax maintenance, product retirement, supply-chain disruption and compliance enforcement individually needs five columns on the hashed `financials` section and would move the envelope 7 → 8; not done, with a test that fails if the version moves.** Open for the owner. |
| W-CE3-05..20 and the W-CE2 residues | **All repaired, pending closure** on `walk-ce3-display-and-language`. W-CE3-15 was the R32 guard firing as ruled, with nothing on the leaderboard saying so; the row now carries the reason from R34's stored payload. **The record's diagnosis of W-CE3-11 was wrong:** `engine/briefing.py` has no caller and cannot run; the English a student read came from `narratives._build_briefing_fields`, whose Chinese templates were complete and never reached. The briefing is re-rendered for the reader from figures it already stores; no stored row is written and no envelope moves. |

**New findings recorded, not repaired:** the "Generation 2 must be active" requirement is enforced nowhere on the write path (enforcing it would be a new rule, R48); `engine/briefing.py` should be wired up or deleted; the walkthrough harness sets a student's language only in the browser and never on the server, which is why team 3's stored Phase-2 artefacts were English — the next walkthrough must set it through the product before round 1 resolves.
