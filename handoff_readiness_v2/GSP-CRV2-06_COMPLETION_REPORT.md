...
| **Scenario catalogue visibility** | **Adopted rule, not an open question** | Supplier, lane, trade-finance-instrument and compliance catalogues are authenticated scenario reference data available from round 1. Progressive disclosure governs team decision fields and stored team decision values, not the existence or contents of shared catalogues; visibility is symmetric across competitors and carries no team-specific decision value |
...
| **Trade finance value conservation** | **Measured; conserves value** | Instrument cycled with the trade held constant: 0.00 across every round, `letter_of_credit` proved persisted on the intended row. Run on a **capable fixture populated from the authoritative `consumer_electronics_2026` definition through the scenario loader** — 6 instruments, 25 suppliers, 20 lanes, 5 regimes. An earlier fixture took the first available scenario, `clean_energy_tech_2026`, which declares neither instruments nor suppliers; that was a fixture-selection defect and never a property of the authoritative scenario. `fixture_contract.py` now asserts every claimed decision family has at least one legal value before a probe measures anything |
