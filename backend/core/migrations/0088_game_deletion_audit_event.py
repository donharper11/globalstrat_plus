...
class DeletionRecordsBlockDowngrade(Exception):
    """Reversing 0088 would drop the only record that a game was deleted."""


def remove_guards(apps, schema_editor):
    # Reversal runs this first (it is the last operation), before the table
    # drops. Each row is the sole durable record of a deleted game (R45): the
    # game itself is gone, so nothing else could reconstruct it. Refuse, and
    # change nothing, while any exists.
    total = apps.get_model('core', 'GameDeletionAuditEvent').objects.count()
    if total:
        raise DeletionRecordsBlockDowngrade(
            f'REFUSED: cannot reverse core.0088_game_deletion_audit_event. '
            f'{total} game deletion record(s) are stored, and each is the only '
            f'durable record that its game was deleted (R45). Nothing was '
            f'changed. Take a competition backup and export those rows before '
            f'deciding to retry; run `manage.py migrate core` to return to head.')
