...
const LogisticsPage = () => {
  ...
  const classifications = ['processing_trade', 'general_trade', 'bonded_logistics']
    .map((value) => ({ value, label: t(`sc.logistics.${value}`) }));
  ...
  const validate = () => {
    const errs = [];
    configuredLaneIds.forEach((id) => { const s = modeSum(id); if (s !== 100) errs.push(t('sc.logistics.validation_mix', { lane: laneById[id]?.lane_id || id, total: s })); });
    return errs;
  };
  ...
  const configuredColumns = [
    { title: t('sc.logistics.lane'), key: 'l', render: (_, r) => <><Text strong>{r.label}</Text><br /><Text type="secondary" style={{ fontSize: 11 }}>{r.code}</Text></> },
    { title: t('sc.logistics.how_ships'), dataIndex: 'split' },
    { title: t('sc.logistics.total'), dataIndex: 'sum', width: 90, render: (v) => <Tag color={v === 100 ? 'green' : 'red'}>{v}% {v !== 100 && '⚠'}</Tag> },
    { title: '', key: 'x', width: 130, render: (_, r) => (<Space>
      <Button size="small" icon={<EditOutlined />} disabled={!editable} onClick={() => setEditLane(r.id)}>{t('sc.common.edit')}</Button>
  ...
  return (
    <div style={{ maxWidth: 1000, width: '100%' }}>
      ...
      <PanelCard headerColor="decision" title={<Space>{t('sc.logistics.shipping_routes')} {round < UNLOCK.modal_mix && lockTag(UNLOCK.modal_mix, t)}</Space>} style={{ marginBottom: 16 }}>
        <Space style={{ width: '100%', justifyContent: 'space-between', marginBottom: 8 }} align="start">
          <Paragraph type="secondary" style={{ fontSize: 12, margin: 0, maxWidth: 620 }}>{t('sc.logistics.routes_help')}</Paragraph>
          <Button size="small" type="primary" icon={<PlusOutlined />} disabled={!editable || round < UNLOCK.modal_mix} onClick={() => setEditLane('new')}>{t('sc.logistics.add_route')}</Button>
        </Space>
        {configuredRows.length === 0
          ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={t('sc.logistics.no_routes')} />
          : <Table rowKey="id" size="small" pagination={false} columns={configuredColumns} dataSource={configuredRows} />}
      </PanelCard>

      <PanelCard headerColor="strategic" title={<Space>{t('sc.logistics.terms_by_market')} {round < UNLOCK.incoterms && lockTag(UNLOCK.incoterms, t)}</Space>} style={{ marginBottom: 16 }}>
        {markets.length === 0 ? <Empty description={t('sc.logistics.no_markets')} /> : (
          <Table rowKey="id" size="small" pagination={false} dataSource={markets}
            columns={[
              { title: t('sc.logistics.market'), key: 'm', render: (_, mk) => <><Text strong>{mk.name}</Text> <Tag>{mk.code}</Tag></> },
              ...
              { title: t('sc.logistics.insurance_pct'), key: 'ins', render: (_, mk) => (<InputNumber min={0} max={200} value={inco[mk.id]?.insurance_coverage_pct} disabled={!editable || round < UNLOCK.insurance_coverage_pct} onChange={(v) => setInco((p) => ({ ...p, [mk.id]: { ...(p[mk.id] || {}), insurance_coverage_pct: v } }))} />) },
            ]} />)}
      </PanelCard>

      <PanelCard headerColor="neutral" title={<Space>{t('sc.logistics.customs_returns')} {round < UNLOCK.customs_classification && lockTag(UNLOCK.customs_classification, t)}</Space>} style={{ marginBottom: 16 }}>
        {markets.length === 0 ? <Empty description={t('sc.logistics.no_markets')} /> : (
          <Table rowKey="id" size="small" pagination={false} dataSource={markets}
            columns={[
              { title: t('sc.logistics.market'), key: 'm', render: (_, mk) => <><Text strong>{mk.name}</Text> <Tag>{mk.code}</Tag></> },
              { title: t('sc.logistics.customs_type'), key: 'cls', render: (_, mk) => (<Select style={{ width: 160 }} allowClear placeholder="—" value={customs[mk.id]?.classification} disabled={!editable || round < UNLOCK.customs_classification} options={classifications} onChange={(v) => setCustoms((p) => ({ ...p, [mk.id]: { ...(p[mk.id] || {}), classification: v } }))} />) },
              { title: t('sc.logistics.returns_capacity'), key: 'rl', render: (_, mk) => (<InputNumber min={0} max={100} value={customs[mk.id]?.reverse_logistics_capacity_pct} disabled={!editable || round < UNLOCK.reverse_logistics} onChange={(v) => setCustoms((p) => ({ ...p, [mk.id]: { ...(p[mk.id] || {}), reverse_logistics_capacity_pct: v } }))} />) },
              { title: t('sc.logistics.returns_hub'), key: 'hub', render: (_, mk) => (<Select style={{ width: 140 }} allowClear placeholder="—" value={customs[mk.id]?.reverse_logistics_hub_market} disabled={!editable || round < UNLOCK.reverse_logistics} options={markets.map((m2) => ({ value: m2.id, label: m2.code }))} onChange={(v) => setCustoms((p) => ({ ...p, [mk.id]: { ...(p[mk.id] || {}), reverse_logistics_hub_market: v } }))} />) },
            ]} />)}
      </PanelCard>

      {/* Add / edit route modal */}
      <Modal open={!!editLane} title={editLane === 'new' ? t('sc.logistics.add_shipping_route') : t('sc.logistics.edit_shipping_route')} width={640}
        onCancel={() => setEditLane(null)}
        footer={[
          <Tag key="t" color={editSum === 100 ? 'green' : (editSum === 0 ? 'default' : 'red')} style={{ marginRight: 'auto' }}>{t('sc.logistics.shipments_pct', { total: editSum })}</Tag>,
          <Button key="c" onClick={() => setEditLane(null)}>{t('sc.common.close')}</Button>,
          <Button key="s" type="primary" loading={saving} disabled={!editable || editLane === 'new'} onClick={saveAndCloseModal}>{t('sc.common.save_close')}</Button>,
        ]}
        styles={{ footer: { display: 'flex', alignItems: 'center', gap: 8 } }}>
        <div style={{ marginBottom: 12 }}>
          <div style={{ marginBottom: 4 }}><Text strong>{t('sc.logistics.route_from_to')}</Text></div>
          <Select style={{ width: '100%' }} showSearch optionFilterProp="label" placeholder={t('sc.logistics.choose_route')}
            value={editLane === 'new' ? undefined : editLane} disabled={editLane !== 'new'} options={laneOptions}
            onChange={(v) => setEditLane(v)} />
        </div>
        {el && (
          <>
            <div style={{ marginBottom: 4 }}><Text strong>{t('sc.logistics.how_does_ship')}</Text> <Text type="secondary" style={{ fontSize: 12 }}>{t('sc.logistics.must_total')}</Text></div>
            <Space wrap size="large">
              {MODES.map((mode) => {
                const avail = modeAvailable(el, mode);
                return (
                  <div key={mode}>
                    <div style={{ fontSize: 12, color: avail ? undefined : '#bbb' }}>{t(`sc.logistics.mode_${mode}`)}</div>
                    <Tooltip title={!avail ? t('sc.logistics.not_available') : ''}>
